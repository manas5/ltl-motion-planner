...
def generate_ts(ltlSpec, sim, robot, eta=[0.5, 1.0],
    ...
    sim.offline = RRGPlanner(robot, checker, iterations=100, eta=eta)
    suffix_cost = None
    ...
    for session in range(1,11):
        ...
        with Timer(op_name='Session',template='"%s runtime": %f ms'):
            ...
            if found:
                ...
                sim.offline.checker.g.clear()
                sim.offline = RRGPlanner(robot, sim.offline.checker, iterations=100, eta=eta)
                E = sim.offline.checker.createPA(prefix, suffix, robot)
                sim.offline.add_to_ts(E)
...
if __name__ == '__main__':
    outputdir = os.path.abspath('../data/cs1/graph_clear')
    setup(outputdir)
    robot, sim, ltlSpec = define_problem(outputdir)
    np.random.seed(1001)
    with Timer(op_name='Global Planning',template='%s runtime: %f ms'):
        generate_ts(ltlSpec, sim, robot, eta=[0.048,0.169], ts_file='ts.yaml')
