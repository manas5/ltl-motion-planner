...
def generate_ts(ltlSpec, sim, robot, eta=[0.5, 1.0],
    ...
    sim.offline = RRGPlanner(robot, checker, iterations=1000, eta=eta)
    suffix_cost = None
    found = sim.offline.solve(finite=True)
    ...
    # sim.display(solution=([v[0] for v in prefix],[v[0] for v in suffix]), save='global_sol_')
...
if __name__ == '__main__':
    outputdir = os.path.abspath('../data/cs1/finite')
    setup(outputdir)
